		...
		function frame() {
			...
			device.queue.writeBuffer(
				...
				new Float32Array([
					...
					inputData.color,
					inputData.noise,
					inputData.chromaticAberration,
					inputData.iterations,
					inputData.power,
					inputData.normals,
					inputData.randomness,
					inputData.wobble,
					inputData.seed,
				])
			);
			...
			device.queue.writeBuffer(
				inputBuffer,
				204 + offset,
				new Float32Array(inputData.colors[0])
			);
			device.queue.writeBuffer(
				inputBuffer,
				204 + offset + 16,
				new Float32Array(inputData.colors[1])
			);
			device.queue.writeBuffer(
				inputBuffer,
				204 + offset + 32,
				new Float32Array(inputData.colors[2])
			);
...
{#if webgpuCapable}
	<div>
		<span>
			<Slider
				startValue={50}
				on:slide={(v) => (inputData.iterations = v.detail.value * 20)}
			/>
		...
		<span>
			<Slider
				startValue={16}
				on:slide={(v) => (inputData.power = v.detail.value * 50)}
			/>
			<h6>fractal_power</h6>
		</span>
		<span>
			<Slider
				startValue={0}
				on:slide={(v) => (inputData.color = v.detail.value)}
			/>
			<h6>color</h6>
			<h6
				on:click={() => (inputData.colors = randomColors())}
				class="bg-inverse"
			>
				[randomize]
			</h6>
		...
		<span>
			<Slider
				startValue={15}
				on:slide={(v) => (inputData.noise = v.detail.value)}
			/>
			<h6>noise</h6>
		</span>
	</div>
	<div class="right">
		<span>
			<Slider
				startValue={25}
				on:slide={(v) =>
					(inputData.chromaticAberration = v.detail.value)}
			/>
		...
		<span>
			<Slider
				startValue={0}
				on:slide={(v) => (inputData.normals = v.detail.value)}
			/>
		...
		<span>
			<Slider
				startValue={0}
				on:slide={(v) => (inputData.randomness = v.detail.value / 10)}
			/>
			<h6>randomness</h6>
			<h6
				on:click={() => (inputData.seed = Math.random() * 100.0)}
				class="bg-inverse"
			>
				[reseed]
			</h6>
		...
		<span>
			<Slider
				startValue={0}
				on:slide={(v) => (inputData.wobble = v.detail.value / 20)}
			/>
			<h6>wobble_speed</h6>
		</span>
	</div>
	<h6
		style="position: absolute; bottom: 0; width: 100vw; text-align: center; margin: 0; transform: translateY(-10px)"
	>
		- x -
	</h6>
{:else}
	<h6
		style="color: black; justify-content: center; display: flex; position: absolute; bottom: 0; width: 100vw"
	>
		No GPU adapter found! Try updating chrome and enabling the experimental
		flag "#enable-unsafe-webgpu" under "chrome://flags.
	</h6>
{/if}
